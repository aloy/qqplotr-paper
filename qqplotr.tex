...
\tightlist
\item
  Standardize colors and sizes for the plots.
\item
  Code organization.
\end{itemize}
...
{\centering \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/ex-qq1-1} \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/ex-qq1-2} 
...
theoretical distribution is \(x = 0\). \citet{Loy2016-fg} find that
...
graphics \citep{R}. \pkg{lattice} package provides a general framework
for Q-Q plots in the \texttt{qqmath} function, allowing one to compare a
sample to any theoretical distribution by specifying the appropriate
quantile function \citep{lattice}. \texttt{qqPlot} in the \pkg{car}
package also allows for the assessment of non-normal distributions and
adds pointwise confidence bands via normal theory or the parametric
bootstrap \citep{car}. \pkg{ggplot2} package provides \texttt{geom\_qq}
and \texttt{geom\_qq\_line}, enabling the creation of Q-Q plots with a
...
\pkg{qqplotr} package extends \pkg{ggplot2} to provide a complete
...
\item
  ...
  nomenclature outlined in \pkg{stats}. For example, for
  \texttt{"custom"}, you must provide the appropriate \texttt{dcustom},
...
\item
  ...
  available in \pkg{stats}, so if a custom distribution is provided to
  \texttt{distribution}, then \emph{all} of its parameters have to be
  properly estimated and passed as a named list on to \texttt{dparams}.
\item
...
\item
  ...
  line that intersects the two data quantiles specified by \code{qprobs}
...
\item
  ...
  which are used to specify which quantiles that the Q-Q line will be
  intercepting.
\end{itemize}
...
\texttt{identity}, all other parameters will have the exact same
interpretation. For \texttt{stat\_qq\_line}, the \texttt{identity}
parameter is \emph{always} used, regardless of the value of
\texttt{detrend}. This parameter controls which reference line is drawn:
...
\item
  ...
  drawn. By default the Q-Q line is drawn through two
  points---corresponding to the .25 and .75 quantiles of the
  distributions on \(x\) (theoretical distribution) and \(y\) (empirical
  distribution). This line estimates the empirical distribution. Using
  robust estimates for the Q-Q line is of particular advantage for small
  samples \citep{Loy2016-fg}.
\item
...
{\centering \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/qqline-1} \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/qqline-2} \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/qqline-3} \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/qqline-4} 
...
\item
  ...
  \emph{both} of themwill be estimated using robust estimates via the
  \pkg{robustbase} package \citep{robustbase}.
\end{itemize}
...
the groups (given that \texttt{dparams\ =\ list()}, which is the default
...
estimates must be manually calculated and passed on to \texttt{dparams}
...
provide multiple examples of how the package can be used. We start by
loading the package:
...
noted, the theoretical distributional parameters are automatically
estimated with MLEs. The shaded region represents the area between the
normal pointwise confidence bands. As we can see, the urine pH
measurements distribution for this dataset is lightly right-skewed.

As previously discussed, three distinct approaches are availble in
\pkg{qqplotr} to construct the confidence bands. The top row of plots in
Figure \ref{fig:urine-qq-bands} presents three configurations with the
different confidence bands constructed over the same normal Q-Q points
and Q-Q lines. In contrast, the bottom row of plots presents the
adjusted detrended versions of the corresponding plots directly above
them.
...
{\centering \includegraphics[width=\textwidth]{qqplotr_files/figure-latex/urine-qq-bands-1} 
...
{\centering \includegraphics[width=0.45\linewidth]{qqplotr_files/figure-latex/jump-density-1} 
...
{\centering \includegraphics[width=0.45\linewidth]{qqplotr_files/figure-latex/sev-qq-1} 
...
{\centering \includegraphics[width=0.45\linewidth]{qqplotr_files/figure-latex/detrend-sev-1} 
...
{\centering \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/heights-1} \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/heights-2} 
...
(of the whole population) each group is compared to. While the mean is
...
to be reasonable, with the exception of a few extreme outliers.

\begin{Schunk}
\begin{figure}

{\centering \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/weights-1} \includegraphics[width=0.4\linewidth]{qqplotr_files/figure-latex/weights-2} 
...
default, the MLEs are used to parameterize a lognormal distribution for
each group:
...
{\centering \includegraphics[width=0.9\linewidth]{qqplotr_files/figure-latex/weights-log-1} 
...
quartiles. While those are the most traditional used reference lines,
alternative ones can be quickly implemented using
\texttt{ggplot2::geom\_abline} by specifying the slope and intercept.
