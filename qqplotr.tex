...
as the \(x\)- and \(y\)-axes show the same range. Adjusting the aspect
ratio in this way ensures that distances in the \(x\)- and
\(y\)-directions are on the same scale. This Q-Q plot design is called
an \emph{adjusted detrended Q-Q plot}. If the range of the axes are not
adjusted, then an ordinary detrended Q-Q plot is produced, which was
found to have lower power than the standard Q-Q plot in some situations
\citep{Loy2016-fg}. Figure \ref{fig:ex-detrend} displays the Q-Q plot
from Figure \ref{fig:ex-qq2} along with its adjusted detrended version.
...
\item
  ...
  details, we refer the reader to the \nameref{sec:detrending} section.
\item
...
which reference line that is drawn:
...
\item
  ...
  tail-sensitive confidence bands. If \texttt{NULL}, then both the
  center and scale parameters will be estimated using robust estimates
  via \pkg{robustbase} \citep{robustbase}.
\end{itemize}
...
each group, then the estimates must be manually calculated and passed
onto \texttt{dparams} as a named list for each of the desired
...
conclude by looking at the tail-sensitive plot at the bottom-left that,
