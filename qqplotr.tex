...
stages. Q-Q plots compare two distributions by matching a commmon set of
quantiles. Additional graphical elements are often added to Q-Q plots in
order to aid in distributional assessment. Different orientations of Q-Q
plots have also been proposed, most notably the detrended Q-Q plot.
Various implementations of Q-Q plots exist in R. \pkg{qqplotr} extends
\pkg{ggplot2} to provide a complete implementation of Q-Q plots. In the
remainder of this paper, we will introduce the plotting framework
provided by \pkg{qqplotr} and provide multiple examples of how it can be
used.
}
...
\begin{itemize}
\item
  Conclusion \textbackslash{}al\{AL: Do we need to add more than we have
  in the discussion already?
\item
  Be consistent with:
  ...
  \item
    Use ``normal'' when not specifying (e.g., ``a normal
    distribution''). Use ``Normal'' when specifying (e.g., ``the
    Standard Normal Distribution'')
  \item
    Use ``colour'' instead of ``color'' when using ggplot2 functions due
    to compatibility reasons.
  \item
    Dashing ponctuation: use em dashes (---) instead of en dashes (--).
...
line is often drawn
\Aa{either by tracing the identity line or by connecting two data quantiles, such as the first and third quartiles}.
...
error for the proposed model. Such bands help gauge how troubling a
departure from the proposed model may be. Figure \ref{fig:ex-qq2} adds
\Aa{identity} reference lines and 95\% pointwise confidence bands to the
Q-Q plots in Figure \ref{fig:ex-qq1}.
...
as \(x\)- and \(y\)-axes show the same range. Adjusting the aspect ratio
in this way ensures that distances in the \(x\)- and \(y\)-directions
are on the same scale. This Q-Q plot design is called an \emph{adjusted
detrended Q-Q plot}. If the range of the axes are not adjusted, then an
ordinary detrended Q-Q plot is produced, which was found to have lower
power than the standard Q-Q plot in some situations \citep{Loy2016-fg}.
Figure \ref{fig:ex-detrend} displays the Q-Q plot from Figure
...
\nameref{sec:background}). Note that all other transformations in the
\pkg{qqplotr} package also allow for the detrend option. Below we
present a complete call to \texttt{stat\_qq\_point} and highlight the
...
\item
  ...
  have to be estimated somehow and included in \texttt{dparams}.
\item
...
\item
  ...
  paraameter is a logical that controls if the identity line should be
  ...
  according to the traditional Q-Q line that intercepts two data
...
Note that all the parameters for \texttt{stat\_qq\_line} are identical
to those for \texttt{stat\_qq\_point}. The only exception being
\texttt{identity}, which have a slightly different interpretation. Here,
\texttt{identity} is \emph{always} used, despite the value of
\texttt{detrend}. This parameter controls which reference line will be
drawn. Two lines are of particular importance in a Q-Q plot:
...
\item
  ...
  theoretical distribution (\texttt{identity\ =\ TRUE}).
\item
  The \emph{Q-Q line} is an estimate of the empirical distribution
  (\texttt{identity\ =\ FALSE}).
\end{enumerate}
...
\item
  ...
  \item
    ...
    referene line, \(\Phi^{-1}(\cdot)\) denotes the quantile function
...
\item
  ...
  tail-sensitive confidence bands. If any of them is \texttt{NULL}, then
  both the center and scale parameters will be estimated using robust
  estimates via \pkg{robustbase} \citep{robustbase}.
\end{itemize}
...
user wants to fit the \emph{same} distribution (i.e., same parameter
estimates) to each group, then the estimates must be manually calculated
and passed onto \texttt{dparams} as a named list for each of the desired
\pkg{qqplotr} transformations.
...
transformations, let us use as example the \texttt{urine} dataset from
\pkg{boot}. This small dataset presents 79 urine specimens that were
analyzed in order to determine if certain physical characteristics of
the urine (e.g., pH or urea concentration) might be related to the
...
is lightly right-skewed. The chosen normal confidence bands suggests
that the assumption of normality is not appropriate to the data.

As previously discussed, three distinct approaches are availble in
\pkg{qqplotr} to construct the confidence bands. The top row from Figure
...
detrended version of the those plots, which will be used to aid in the
visual inference.

When assessing the tail-sensitive confidence bands (bottom-right), we
can see that an additional point is inside the confidence bands than
with the normal (bottom-left) and the bootstrap confidence bands methods
(bottom-center). In addition, the points from the distribution center of
mass are also more distant tail-sensitive confidence bands limits than
in the other methods. Hence, one could conclude by looking at the
tail-sensitive plot at the bottom-left that, even though the normal
distribution seems like a slightly rough approximation, the assumption
...
\caption{Normal Q-Q plots of pH measurements from urine samples using different confidence bands. Depending on the type of confidence band used, we come to different conclusions. \Aa{AA: Not sure about how much (and if) to include of the code chunk used to produce}}\label{fig:urine-qq-bands}
\end{figure}
\end{Schunk}
...
\al{(We need a citation for this quote.)}\\
...
To further illustrate the functionality of \pkg{qqplotr}, we focus on
...
\al{We need to add the code! Should we show the entire chunk below?}
...
detrended orientations, reference lines for Q-Q plots, and confidence
bands for Q-Q plots. The examples illustrate how to create Q-Q plots for
non-standard distributions found outside of the \pkg{stats}, how to
...
We have made design choices in \pkg{qqplotr} that we believe are in line
...
\Aa{implements a reference line that passes through two quantiles of the distributions -- by default, the first and third quartiles, but this is easily changable by using the `qprobs` argument.}
While this is the most traditional used reference line, alternative
reference lines
\Aa{that are not based on quantiles of the distributions} can be quickly
