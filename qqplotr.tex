...
plots have also been proposed, most notably the de-trended Q-Q plot.
...
\item
  ...
  outlined in the \pkg{stats} package (i.e., for \texttt{"custom"}, you
...
\item
  ...
  function from the \pkg{stats} package, which is used to define the
...
\begin{Schunk}
\begin{figure}
\includegraphics[width=\textwidth]{qqplotr_files/figure-latex/qqline-1} \caption[Q-Q plots of (top-left) a mis-specified Normal distribution, (top-right)  a Normal distribution with ML parameter estimates]{Q-Q plots of (top-left) a mis-specified Normal distribution, (top-right)  a Normal distribution with ML parameter estimates. Between these two plots only the scales on the axes change. The bottom two plots show Q-Q plots of the same sample from a log-normal distribution. On the left, the theoretical distribution is mis-specified with a mean of 0 and variance of 1. On the right, ML estimates for mean and variance are used.}\label{fig:qqline}
\end{figure}
\end{Schunk}
...
\caption{Normal Q-Q plot comparing pH measurements of the urine samples with a normal distribution. \al{Do we really need this plot? We could reference Figure 6 as easily.}}\label{fig:urine-qq}
\end{figure}
\end{Schunk}
...
\pkg{qqplotr} to construct the confidence bands. Figure
