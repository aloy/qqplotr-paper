...
Various implementations of Q-Q plots exist in R. Normal Q-Q plots, where a sample is compared to the Standard Normal Distribution, are implemented using `qqplot` and `qqline` in \pkg{base} graphics \citep{R}. The \pkg{lattice} package provides a general framework for Q-Q plots in the `qqmath` function, allowing one to compare a sample to any theoretical distribution by specifying the appropriate quantile function \citep{lattice}. `qqPlot` in the \pkg{car} package also allows for the assessment of non-normal distributions and adds pointwise confidence bands via normal theory or the parametric bootstrap \citep{car}. The \pkg{ggplot2} package provides `geom_qq` and `geom_qq_line`, enabling the creation of Q-Q plots with a reference line, much like those created using `qqmath` \citep{ggplot2}. None of these general-use packages allow for easy construction of detrended Q-Q plots.
...
**AL: I'm having trouble with the first sentence of this paragraph. When using the identity line the slope will be 1, which provides no link to the estimated location or scale parameters. The standardized Q-Q plot will provide graphical estimates of the location and scale parameters.**
...
**AL: I'm not sure what to do here anymore---that is, I have lost the story that we are trying to tell in this subsection. Is our goal to clarify the difference between the bands? to provide a simply first example? to showcase the different designs/configurations? If our goal is to explore the bands, then perhaps cutting the detrending bit makes sense. Also, I am struggling to find a good way to discuss the different bands---is the discussion obvious so we can ditch it?**
...
```{r urine-qq-bands-identity, echo=FALSE, message=FALSE, warning=FALSE, out.width='\\textwidth', fig.align='center', fig.width=9.5, fig.height=7.0, fig.cap="Normal Q-Q plots of pH measurements from urine samples using different confidence bands. Depending on the type of confidence band used, we come to different conclusions."}
library(boot)
set.seed(9)

p31 <- urine %>% ggplot(aes(sample = ph)) + 
  stat_qq_band(bandType = "normal", fill = "#8DA0CB", alpha = 0.4) + 
  stat_qq_line(colour = "#8DA0CB") +
  stat_qq_point() +
  ggtitle("Normal") +
...
p32 <- urine %>% ggplot(aes(sample = ph)) + 
  stat_qq_band(bandType = "boot", fill = "#FC8D62", alpha = 0.4, identity = TRUE) + 
  stat_qq_line(colour = "#FC8D62", identity = TRUE)+
  stat_qq_point() +
  ggtitle("Bootstrap") +
  xlab("Normal quantiles") +
  ylab("pH measurements quantiles") +
...
p33 <- urine %>% ggplot(aes(sample = ph)) + 
  stat_qq_band(bandType = "ts", fill = "#66C2A5", alpha = 0.4, identity = TRUE) + 
  stat_qq_line(colour = "#66C2A5", identity = TRUE) +
  stat_qq_point() +
  ggtitle("Tail-sensitive") +
  xlab("Normal quantiles") +
  ylab("Observed quantiles of pH measurements") +
...
p41 <- urine %>% ggplot(aes(sample = ph)) + 
  stat_qq_band(bandType = "normal", fill = "#8DA0CB", alpha = 0.4, detrend = TRUE, identity = TRUE) + 
  stat_qq_line(colour = "#8DA0CB", detrend = TRUE, identity = TRUE) +
  stat_qq_point(detrend = TRUE, identity = TRUE) +
  ggtitle("Normal (Detrended)") +
  xlab("Normal quantiles") +
  ylab("Differences") +
  theme_light() +
...
p42 <- urine %>% ggplot(aes(sample = ph)) + 
  stat_qq_band(bandType = "boot", fill = "#FC8D62", alpha = 0.4, detrend = TRUE, identity = TRUE) + 
  stat_qq_line(colour = "#FC8D62", detrend = TRUE, identity = TRUE)+
  stat_qq_point(detrend = TRUE, identity = TRUE) +
  ggtitle("Bootstrap (Detrended)") +
  xlab("Normal quantiles") +
  ylab("Differences") +
...
p43 <- urine %>% ggplot(aes(sample = ph)) + 
  stat_qq_band(bandType = "ts", fill = "#66C2A5", alpha = 0.4, detrend = TRUE, identity = TRUE) + 
  stat_qq_line(colour = "#66C2A5", detrend = TRUE, identity = TRUE) +
  stat_qq_point(detrend = TRUE, identity = TRUE) +
  ggtitle("Tail-sensitive (Detrended)") +
  xlab("Normal quantiles") +
  ylab("Differences") +
