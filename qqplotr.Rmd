...
p3 <- sub %>% ggplot(aes(sample = WTKG3/100, colour=Gender, fill=Gender)) + 
  ...
  theme_bw() +
  theme(legend.position = c(0.8, 0.2), aspect.ratio = 1)
p4 <- sub %>% ggplot(aes(sample = WTKG3, colour=Gender, fill=Gender)) + 
  stat_qq_band(alpha = 0.6, distribution="lnorm") + 
...
p5 <- sub %>% ggplot(aes(sample = log(WTKG3), colour=Gender, fill=Gender)) + 
  stat_qq_band(alpha = 0.6) + 
  stat_qq_line() + 
  stat_qq_point() +
  scale_fill_brewer(palette="Set1") +
  scale_colour_brewer(palette = "Set1") +
  xlab("Sample quantiles") +
  ylab("log of Weight (in inch)") +
  theme_bw() + 
  theme(legend.position = c(0.2, 0.8), aspect.ratio=1)
