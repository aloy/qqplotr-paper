...
```
```{r, echo=FALSE, fig.width = 8, fig.height = 3.5, fig.cap = "Sample (200 men and 200 women) of weights (raw data, left; jittered data, right). Unlike people's height, weight seems to have heavier tails than a normal distribution. Jittereing (on the right) emphasizes that the distribution of weight is heavily right skewed.", out.width='\\textwidth', fig.align="center", message=FALSE, warning = FALSE}
p3 <- sub %>% ggplot(aes(sample = WTKG3, colour=Gender, fill=Gender)) + 
  stat_qq_band(alpha = 0.6) + stat_qq_line() + stat_qq_point() +
  scale_fill_brewer(palette="Set1") +
  scale_colour_brewer(palette = "Set1") +
  xlab("Sample quantiles") +
  ...
  theme(legend.position = c(0.8, 0.2), aspect.ratio = 1)
p4 <- sub %>% mutate(
  WTKG3.jitter = jitter(WTKG3, factor = 2)
) %>% ggplot(aes(sample = WTKG3.jitter, colour=Gender, fill=Gender)) + 
  stat_qq_band(alpha = 0.6) + stat_qq_line() + stat_qq_point() +
  scale_fill_brewer(palette="Set1") +
  scale_colour_brewer(palette = "Set1") +
  xlab("Sample quantiles") +
