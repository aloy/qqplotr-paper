...
Close to half a million interviews are conducted each year. Here, we are focussing on the 2012 responses for Iowa. 7166 responses were gathered across 359 questions and derived variables. Among these, are people's height and weight, which we are going to assess using a normal assumption:
...
) %>% unnest(data) %>% select(SEX, WTKG3, HTIN4) %>%
...
p4 <- sub %>% ggplot(aes(sample = WTKG3/100, colour=Gender, fill=Gender)) + 
  stat_qq_band(distribution="lnorm", alpha = 0.6) + 
...
layers <- sub %>% nest(-Gender) %>% mutate(
  band = data %>% purrr::map(.f = function(d) {
    ...
    d %>% geom_qq_band(alpha=0.6, distribution="lnorm", 
  ...
  line = data %>% purrr::map(.f = function(d) {
    ...
    sdlog <- sd(log(d$WTKG3/100), na.rm=TRUE)
    d$Gender <- c("Male", "Female")[d$SEX[1]]
    d %>% stat_qq_line(alpha=0.6, distribution="lnorm", 
                      dparams=list(meanlog = mlog, sdlog = sdlog),
                      aes(colour = Gender))
  }),
  points = data %>% purrr::map(.f = function(d) {
    ...
    sdlog <- sd(log(d$WTKG3/100), na.rm=TRUE)
    d$Gender <- c("Male", "Female")[d$SEX[1]]
    d %>% qqplotr::stat_qq_point(alpha=0.6, distribution="lnorm", 
                      dparams=list(meanlog = mlog, sdlog = sdlog),
                      aes(colour = Gender))
  })
)
