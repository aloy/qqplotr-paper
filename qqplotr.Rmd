...
custom distribution (as we will do in \nameref{sec:user-dists}), you may do
...
Two lines in a Q-Q plot are of particular importance:
...
```{r qqline, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=8, out.width='\\textwidth', fig.cap="Q-Q plots of (top-left) mis-specified Normal distribution, (top-right)  Normal distribution with ML parameter estimates. Between these two plots only the scales on the axes change. The bottom two plots show Q-Q plots of the same sample from a log-normal distribution. On the left, the theoretcial distribution is mis-specified with a mean of 0 and variance of 1. On the right, ML estimates for mean and variance are used."}
library(tidyverse)
library(qqplotr)
library(gridExtra)
...
center and scale distributional parameters, respectively, which are used to
