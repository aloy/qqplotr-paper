...
```{r, echo=FALSE, fig.width = 3.5, fig.height = 3, fig.caption = "Sample (500 men and 500 women) of raw heights. The distribution is dominated by the discreteness of the data.", message=FALSE, warning = FALSE}
library(tidyverse)
iowa <- readRDS("data/brfss-iowa.rda")
sub <- iowa %>% nest(-SEX) %>% mutate(
  data = data %>% purrr::map(.f = function(x) sample_n(x, size = 500))  
) %>% unnest(data) %>% select(SEX, WTKG3, HTIN4) %>%
  mutate(Gender = c("Male", "Female")[SEX])
sub %>% ggplot(aes(sample = HTIN4, colour=Gender, fill=Gender)) + 
