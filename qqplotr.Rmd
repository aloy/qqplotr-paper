...
permitting the drawing of Q-Q and P-P points, lines, and confidence bands. For each one of those 
plot elements, our approach was to implement a \pkg{ggplot2} "stat" (statistical transformation). In
addition, we also implemented a \pkg{ggplot2} "geom" (geometrical object) for the confidence bands, 
which permits a cleaner way of handling graphical parameters. This will become clearer in the
\nameref{sec:examples} section.

For simplicity, we will present separately the Q-Q and P-P functions below.

functions of this package also allow a detrend adjustment of the plots, proposed by Thode (2002) to
help reduce visual bias when assessing the results.
...
To illustrate this process we will consider the smallest exreme value (SEV) distribution. This is a left-skewed distribution, which is a key feature needed in order to model the long jump results. Below, we define the suite of distribution functions needed to utilize the SEV distribution.
