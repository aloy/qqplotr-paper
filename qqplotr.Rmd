...
* Be consistent with:
    ...
    + Are we using em dashes (---) or en dashes (--) throughout the text? With or without spacing?
...
order statistic is \linebreak $\widehat{X}_{(i)}~\pm~\Phi^{-1}(.975)~\cdot~SE(X_{(i)})$,
...
statistic.
    + **Bootstrap:** Specifying `bandType = "boot"` constructs pointwise confidence 
...
`bandType = "boot"`) or the number of simulated samples (if `bandType = "ts"`).
...
p10 <- urine %>% ggplot(aes(sample = ph)) + 
  stat_qq_band(alpha = 0.5, bandType = "boot", fill = "#FC8D62") + 
