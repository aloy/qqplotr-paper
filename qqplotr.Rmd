...
\hh{XXX put in table with population estimates and corresponding estimates by gender.}
...
p7 <- ggplot(longjump, aes(sample = distance)) +
  stat_qq_band(distribution = "sev", alpha = 0.3, dparams=c(mu=0, sigma=1)) +
  stat_qq_line(distribution = "sev", dparams=c(mu=0, sigma=1)) +
  stat_qq_point(distribution = "sev", dparams=c(mu=0, sigma=1)) +
  xlab("Theoretical quantiles") +
  ylab("Jump distance (in m)") +
  theme_bw()
