...
the Q-Q plots in Figure \ref{fig:ex-qq1}. While confidence bands help
...
implementation is the ability to create a detrended version of the
plotted points. All other transformations in \pkg{qqplotr} also allow
for the detrend option. Below we present a complete call to
...
\item
  ...
  detrending---that is, if \texttt{detrend\ =\ TRUE}. If
...
\item
  ...
  \tightlist
  \item
    \textbf{Simultaneous}: Specifying \texttt{bandType\ =\ "ks"}
...
implementation allows you to specify the suffix (i.e., distribution or
abbreviation) via the \texttt{distribution} argument and the parameter
estimates via the \texttt{dparams} argument. However, there are times
when the distributions in \pkg{stats} are not sufficient for the demands
of the analysis. For example, there is no left-skewed distribution
listed aside from the Beta Distribution, which has a restrictive
support. User-coded distributions, or distributions from other packages,
can be used with \pkg{qqplotr} as long as the distributions are defined
following the conventions laid out in \pkg{stats}. Specfically, for some
distribution there must be density/mass (\texttt{d} prefix), CDF
(\texttt{p} prefix), quantile (\texttt{q} prefix), and simulation
(\texttt{r} prefix) functions. In this section, we illustrate the use of
the smallest extreme value distribution (SEV).
...
While \pkg{qqplotr} provides a complete implementation of the Q-Q plot,
